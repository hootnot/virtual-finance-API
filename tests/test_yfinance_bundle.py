...
class TestCompatYfinance(unittest.TestCase):
    ...
    @parameterized.expand([
        (yfe.Profile, 'calendar', '_yf_profile_calendar', 'yahoo_profile.raw',
            ['calendarEvents', 'upgradeDowngradeHistory', 'esgScores']),
        (yfe.Profile, 'recommendations', '_yf_profile_recommendations',
            'yahoo_profile.raw', []),
        (yfe.Profile, 'sustainability', '_yf_profile_sustainability',
            'yahoo_profile.raw', []),
        (yfe.Profile, ('info', None), '_yf_profile_info',
            'yahoo_profile.raw', []),
        (yfe.Holders, 'major', '_yf_holders_major', 'yahoo_holders.raw', []),
        (yfe.Holders, 'mutualfund', '_yf_holders_mutualfund',
            'yahoo_holders.raw', []),
        (yfe.Holders, 'institutional', '_yf_holders_institutional',
            'yahoo_holders.raw', []),
    ])
    ...
    def test__yf_history001(self, mock_req):
        ...
        client.request(r)
        self.assertTrue((r.response == resp) and
                        (list(r.history['Open'].values) == resp['chart']['result'][0]['indicators']['quote'][0]['open']) and  # noqa E501
                        (list(r.history['High'].values) == resp['chart']['result'][0]['indicators']['quote'][0]['high']) and  # noqa E501
                        (list(r.history['Low'].values) == resp['chart']['result'][0]['indicators']['quote'][0]['low']) and  # noqa E501
                        (list(r.history['Close'].values) == resp['chart']['result'][0]['indicators']['quote'][0]['close']) and  # noqa E501
                        (list(r.history['Volume'].values) == resp['chart']['result'][0]['indicators']['quote'][0]['volume']) and  # noqa E501
                        (sorted(list(r.dividends.values)) == sorted([d['amount'] for k, d in resp['chart']['result'][0]['events']['dividends'].items()])) and  # noqa E501
                        (sorted(list(r.splits.values)) == sorted([s['numerator'] / s['denominator'] for k, s in resp['chart']['result'][0]['events']['splits'].items()])))  # noqa E501
